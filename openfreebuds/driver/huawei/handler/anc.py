...
class OfbHuaweiAncHandler(OfbDriverHandlerHuawei):
    ...
    # Single source of truth for the adaptive-transparency slider: both the
    # advertised min/max and the set_property bounds check read from here.
    awareness_level_min = 0
    awareness_level_max = 10
        ...
        if len(data) == 2:
            ...
            if data[1] == 1 and self.w_cancel_lvl:
                ...
                if data[0] == 4 and len(awareness_level) == 1:
                    ...
                    new_props["awareness_level_min"] = str(self.awareness_level_min)
                    new_props["awareness_level_max"] = str(self.awareness_level_max)
    ...
    async def set_property(self, group: str, prop: str, value):
        if prop == "awareness_level":
            level = int(value)
            if not self.awareness_level_min <= level <= self.awareness_level_max:
                raise ValueError(
                    f"Awareness level must be between "
                    f"{self.awareness_level_min} and {self.awareness_level_max}"
                )
