...
class OfbHuaweiPromptToneHandler(OfbDriverHandlerHuawei):
    ...
    async def _prepare_resources(self):
        ...
        try:
            ...
            log.warning("Prompt tone resources rejected: %s", error)
            await self._set_transfer_failed(error.code, str(error))
            return
        except Exception as error:
            log.exception("Prompt tone resource download failed")
            await self._set_transfer_failed(None, f"Could not download case sounds: {error}")
            return
