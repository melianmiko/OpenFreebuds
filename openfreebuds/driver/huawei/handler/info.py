...
class OfbHuaweiInfoHandler(OfbDriverHandlerHuawei):
    ...
    async def on_init(self):
        ...
        resp = await self.driver.send_package(
            HuaweiSppPackage.read_rq(b"\x01\x07", list(range(32)))
        )
    ...
    async def on_package(self, package: HuaweiSppPackage):
        ...
        for key in package.parameters:
            if key == 24 and package.parameters[key][0:2] == b"L-":
                ...
                try:
                    _parse_per_earphone_sn(
                        out, package.parameters[key].decode("utf8", "ignore")
                    )
                except:
                    pass
                continue
            try:
                ...
                for encoding in ["utf8", "gbk", "gb2312", "ascii"]:
                    try:
                        out[self.descriptor.get(key, f"field_{key}")] = (
                            package.parameters[key].decode(encoding)
                        )
                        break
                    except:
                        continue
            except:
                # If all decodings fail, use hex representation
                out[self.descriptor.get(key, f"field_{key}")] = package.parameters[
                    key
                ].hex()
