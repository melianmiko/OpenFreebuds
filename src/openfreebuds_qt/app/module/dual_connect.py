...
from openfreebuds_qt.app.qt_utils import blocked_signals, exec_msg_box_async
    ...
    async def update_ui(self, event: OfbCoreEvent):
        ...
        if event.is_changed("config", "dual_connect"):
            with blocked_signals(self.global_toggle):
                self.global_toggle.setChecked(await self.ofb.get_property("config", "dual_connect") == "true")
